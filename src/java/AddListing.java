...
public class AddListing extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        DormModel model = new DormModel(con, name, userCity, dormAddress, price, space, contact, link);

        
        
        
        try{
            ResultSet output = model.addOutput();

            if(con != null){
                if(output.next()){
                    ResultSet record = model.getAddOutput();
